...
def receive_messages():
    ...
    while True:
        ...
        if packet_type == "kicked_policy":
            ...
            window.after(0, terminate_app, "Hash Error", "Integrity check failed.")
            break
...
def execute_popup_variant(popup_style, popup_name, splits=1):
    ...
    for i in range(splits):
        ...
        if "meme_2019_stonks" in popup_style:
            ...
            canvas.create_text(cx, img_y1 + ((img_y2 - img_y1) // 2), text="MEME", fill=ACCENT_BLUE, font=("Segoe UI", max(5, int(size_factor*0.055)), "bold"), width=draw_w - (inner_gap_w * 3), justify=tk.CENTER)
...
username_entry = tk.Entry(reg_win, bg=BG_BOX, fg=FG_TEXT, font=("Segoe UI", 10), justify=tk.CENTER, bd=0, highlightthickness=1, highlightbackground=BORDER_COLOR, insertbackground="black")
...
def register_profile():
    ...
    if entered:
        ...
        messagebox.showwarning("Incomplete", "Username is required.")


tk.Button(reg_win, text="Connect", bg=ACCENT_BLUE, fg="white", font=("Segoe UI", 10, "bold"), bd=0, width=22, command=register_profile).pack(pady=15)
reg_win.mainloop()
...
try:
    client_hash = calculate_file_hash()
    send_packet(server, {
        "type": "handshake",
        "name": name,
        "hash": client_hash
    ...
    print("Registration sequence terminated.")
    exit()
